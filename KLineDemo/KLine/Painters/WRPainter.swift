...
class WRPainter: CALayer, Painter {
    ...
    static func draw(to layer: CALayer, area: CGRect, models: [KLine], minMax: KLineMinMax) {
        ...
        for (i, m) in models.enumerated() {
            ...
            let point1 = CGPoint(x: x + w / 2, y: maxH - (CGFloat(m.wr.wr1) - minMax.min) * unitValue)
            let point2 = CGPoint(x: x + w / 2, y: maxH - (CGFloat(m.wr.wr2) - minMax.min) * unitValue)
            if i == 0 {
    ...
    static func getMinMaxValue(models: [KLine]) -> KLineMinMax {
        ...
        for m in models {
            maxAssert = .maximum(m.wr.wr1, .maximum(m.wr.wr2, maxAssert))
            minAssert = .minimum(m.wr.wr1, .minimum(m.wr.wr2, minAssert))
        }
