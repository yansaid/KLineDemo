...
class BOLLPainter: CALayer, Painter {
    ...
    static func draw(to layer: CALayer, area: CGRect, models: [KLine], minMax: KLineMinMax) {
        ...
        for (i, m) in models.enumerated() {
            ...
            let point1 = CGPoint(x: x + w / 2, y: maxH - (CGFloat(m.boll.up) - minMax.min) * unitValue)
            let point2 = CGPoint(x: x + w / 2, y: maxH - (CGFloat(m.boll.mid) - minMax.min) * unitValue)
            let point3 = CGPoint(x: x + w / 2, y: maxH - (CGFloat(m.boll.low) - minMax.min) * unitValue)
            if i == 0 {
    ...
    static func getMinMaxValue(models: [KLine]) -> KLineMinMax {
        ...
        for m in models {
            maxAssert = .maximum(m.boll.low, .maximum(m.boll.up, .maximum(m.boll.mid, maxAssert)))
            minAssert = .minimum(m.boll.low, .minimum(m.boll.up, .minimum(m.boll.mid, minAssert)))
        }
