...
class KDJPainter: CALayer, Painter {
    ...
    static func draw(to layer: CALayer, area: CGRect, models: [KLine], minMax: KLineMinMax) {
        ...
        for (i, m) in models.enumerated() {
            ...
            let point1 = CGPoint(x: x + w / 2, y: maxH - (CGFloat(m.kdj.k) - minMax.min) * unitValue)
            let point2 = CGPoint(x: x + w / 2, y: maxH - (CGFloat(m.kdj.d) - minMax.min) * unitValue)
            let point3 = CGPoint(x: x + w / 2, y: maxH - (CGFloat(m.kdj.j) - minMax.min) * unitValue)
            if i == 0 {
    ...
    static func getMinMaxValue(models: [KLine]) -> KLineMinMax {
        ...
        for m in models {
            maxAssert = .maximum(m.kdj.j, .maximum(m.kdj.d, .maximum(m.kdj.k, maxAssert)))
            minAssert = .minimum(m.kdj.j, .minimum(m.kdj.d, .minimum(m.kdj.k, minAssert)))
        }
